...
var validator = (function($){
    ...
    */
    mark = function( field, text ){
        ...
        var custom_prompt = field.attr("valid-prompt") ? field.attr("valid-prompt") : text

        if( item.hasClass(defaults.classes.bad) ){
            if( defaults.alerts )
                item.find('.'+defaults.classes.alert).html(custom_prompt);
        }


        else if( defaults.alerts ){
            warning = $('<div class="'+ defaults.classes.alert +'">').html( custom_prompt );
            item.append( warning );
        }
